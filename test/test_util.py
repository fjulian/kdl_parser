...
class TestModelFitting(unittest.TestCase):
    ...
    def test_line_fitting(self):
        ...
        points_line = []
        for i in range(50):
            new_point = line_start + np.random.uniform(-5.0, 5.0) * line_direction + np.random.normal(0.0, 0.01, 3)
        ...
        estimated_line_start2, estimated_line_direction2 = alt_fit_line_3d(points_line, False)
        ...
        ax.plot(estimated_points[0,:], estimated_points[1,:], estimated_points[2,:], label='est')
        ax.plot(estimated_points2[0,:], estimated_points2[1,:], estimated_points2[2,:], label='est2')
        ax.legend()
